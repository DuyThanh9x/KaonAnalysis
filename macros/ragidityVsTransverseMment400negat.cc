...
void ragidityVsTransverseMment(vector<TString> filename,vector<double> kw,vector<double> kwind1, vector<double> kwind2, vector<double> piw,vector<double> piwind1, vector<double> piwind2, TH2D *&rkaon, TH2D *&rpion)
{
        double mss2 = -1;
                      double  kmss2=TMath::Sq(0.4937);
                         rkaon= new TH2D("rkaon","Phase space distribution of K^{-};y;pt [GeV/Q]",200,0.4,2.8,200,0,1.5);
                       double pimss2=TMath::Sq(0.1396);
                         rpion= new TH2D("rpion","Phase space distribution of #pi^{-};y;pt [GeV/Q]",200,0.,3.8,200,0,1.5);
	...
	for (int i = 0; i <data->GetEntries();i++) {
                data->GetEntry(i);
		if (i% 500000 == 0) cout<<">>>> Nevents Access "<<i<<endl;
...
void ragidityVsTransverseMment400negat(vector<TString> filename,TString particleIDfile)
{
	...
	TFile *out = new TFile("25/histragidity400negat.root","recreate");
	...
	out->WriteObjectAny(v,"TGraph",name[i]+"Momentum0GeV6");
