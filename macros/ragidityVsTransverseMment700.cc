                ...
                vector<double> prw,vector<double> prwind1, vector<double> prwind2, TH2D *&rkaon, TH2D *&rpion,TH2D *&rproton)
{
                       double kmss2=TMath::Sq(0.4937);
                         rkaon= new TH2D("rkaon","Phase space distribution of K^{+};y;pt [GeV/Q]",200,0.5,2.5,200,0,1.2);
                       double pimss2=TMath::Sq(0.1396);
                         rpion= new TH2D("rpion","Phase space distribution of #pi^{+};y;pt [GeV/Q]",200,0.6,3.8,200,0,1.2);
               double prmss2=TMath::Sq(0.9383);
                 rproton= new TH2D("rproton","Phase space distribution of p;y;pt [GeV/Q]",200,0.2,1.8,200,0,1.2);
	...
	int ntrack = 0;
        ...
        double radiusOfTarget = 1.2;//cout<<">>>> Analysing data in file "<<filename[id]<<endl;
        ...
        data->SetBranchAddress("Hit700PlaneNumber",&hitz,&bhitz);
...
void ragidityVsTransverseMment700(vector<TString> filename,TString particleIDfile) {
	vector<double> kw; vector<double> kwind1; vector<double> kwind2; vector<double> piw;vector<double> piwind1; vector<double> piwind2; vector<double> prw;vector<double> prwind1; vector<double> prwind2;
        read(particleIDfile,kwind1,kwind2,kw,piwind1,piwind2,piw,prwind1,prwind2,prw);cout<<">>>> "<<kw.size()<<endl;
	TH2D *rk=nullptr,*rpi=nullptr,*rpr=nullptr;
        ragidityVsTransverseMment(filename,kw,kwind1, kwind2, piw,piwind1, piwind2,prw,prwind1,prwind2,rk,rpi,rpr);TFile *out = new TFile("25/histragidity700.root","recreate");
