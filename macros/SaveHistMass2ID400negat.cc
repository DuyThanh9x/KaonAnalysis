void SaveHistMass2ID400negat (vector<TString> filename) {
	...
	TH2D *masv = new TH2D("masv","",250,-.5,1.5,21,0.5,2.6);
	...
	data->SetBranchAddress("TrackTof400Mq2",&m2);
	...
	for (int i = 0; i <data->GetEntries();i++) {
		...
		for (int j=0;j<m2->size();j++) {
			...
			if ((m2->at(j) >= -0.5) && (m2->at(j) <= 1.5) && (p->at(j) <= -0.5) && (p->at(j) >= -2.6) ) {
				masv->Fill(m2->at(j),abs(p->at(j)));
			}
		}
	}
	TFile *out = new TFile("25/histmass400negat.root","recreate");masv->Write();
	for (int i = 1; i < 22; i++) {
                masv->ProjectionX(Form("mass%d",i),i,i);
                TH1D *mass1d = (TH1D*)gDirectory->Get(Form("mass%d",i));
		mass1d->SetTitle(Form("Mq2 distribution at TOF400 with momentum range [%.2f, %.2f] GeV;GeV^{2};#Candidates",.1*(i-1)+.5,.1*i+.5));
		mass1d->Write();
	}
        out->Close();
}
